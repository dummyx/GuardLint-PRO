...
This paper makes the following contributions: (i) it characterizes the \texttt{RB\_GC\_GUARD} omission problem in CRuby as an interaction between derived pointers into object-owned memory, conservative native-stack scanning, and compiler lifetime/stack-slot reuse; (ii) it presents GuardLint, a CodeQL-based static analysis that approximates when a guard is needed using pointer-derivation identification, call-graph reachability to GC entry points, and CFG ordering constraints; (iii) it reports an end-to-end evaluation on CRuby v3.4.5, including candidate omissions, potentially redundant guards, and concrete case studies; and (iv) it quantifies the runtime performance cost of indiscriminate guarding to motivate selective insertion and removal.
...
\textbf{(1) Extraction:} identify candidate owner \texttt{VALUE}s and derived pointers. \\
\textbf{(2) Trigger analysis:} compute call sites whose callee can reach \texttt{gc\_enter}. \\
\textbf{(3) Ordering:} require derivation $\rightarrow$ last \texttt{VALUE}-level use $\rightarrow$ trigger $\rightarrow$ derived-pointer use on the CFG. \\
\textbf{(4) Filtering:} suppress when an explicit guard or a post-trigger \texttt{VALUE}-level access keeps the owner live. \\
\textbf{(5) Reporting:} classify as missing, matched, or potentially redundant under the model.}}
\caption{GuardLint pipeline at a high level.}
\label{fig:guardlint_pipeline}
\end{figure}
...
\paragraph{Rooting analyses in production VMs.}
