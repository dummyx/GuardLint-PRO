...
We plan to improve GuardLint in three main areas. First, we aim to better incorporate domain knowledge of CRuby's implementation, such as encoding specific API patterns like argument parsing with \texttt{rb\_scan\_args} directly into the query. Second, we intend to enhance interprocedural analysis to distinguish cases where a derived pointer is merely passed through from cases where it is dereferenced or stored across GC-triggering calls inside callees. Third, we will incorporate data-flow tracking to determine where pointer values originate: whether from caller-provided storage (likely safe) or from local allocations (requires guarding).
