...
Similarly, pointer derivation operations, GC-triggering functions, and other elements needed by the queries are defined as classes. Defining specific types in this manner limits the upper bound on the size of a relation, helping the query optimizer be more effective~\cite{codeql_specific_types}.

\subsubsection*{Pointer Derivations and Subordinate Relation Binding}
We define predicates that recognize common derivation operations in CRuby. Listing~\ref{lst:derivation_predicate} shows the structure of our derivation detection, which matches against the underlying function name.
...
This class is then used to compose the class of actual derivation function calls. The logical relation between owning objects and subordinate pointers can be represented as: (1) an owning object (\texttt{VALUE}) is passed to a pointer derivation function call, (2) the return value is assigned to a pointer-type variable, and (3) the assigned pointer is considered a subordinate pointer of that owning object. Listing~\ref{lst:derivation_relation} shows the implementation.
