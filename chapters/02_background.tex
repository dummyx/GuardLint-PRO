...
Two common optimizations contribute to the Guard Problem. First, after extracting a subordinate pointer \texttt{p} from a \texttt{VALUE v} (e.g., \texttt{p = RSTRING\_PTR(v)}), the compiler may stop keeping \texttt{v} if it is not referenced again. The register or stack slot that previously held \texttt{v} can then be overwritten, making the object unreachable to conservative scanning.

Second, to reduce stack usage, compilers may reuse the same stack slot for different locals at different times. GCC exposes \texttt{-fstack-reuse} to control such reuse\cite{gcc_fstack_reuse}, and LLVM applies a similar stack-slot reuse optimization\cite{llvm_stack_coloring}. Under conservative scanning, this reuse can overwrite the last remaining copy of a \texttt{VALUE} even while subordinate pointers into its payload are still being used.
