...
\subsection{Finding All GC Triggers}
...
As discussed in the Section \ref{sec:Background}, in order to analyze if a \texttt{VALUE} variable needs to be guarded, we have to identify all the functions that might trigger GC. We define the functions that eventually call CRuby's GC entry point \texttt{gc\_enter} either directly or indirectly.


\subsection{Derivation Patterns}

Subordinate pointers are obtained through many different APIs and macros, each with different signatures and expansion patterns. Some are simple macros that expand to just a function call that takes a \texttt{VALUE} variable and returns it's subordinate pointer like \texttt{RSTRING\_PTR}, while others involve function calls with complex control flow. Table~\ref{tab:derivation_patterns} summarizes the major categories.
